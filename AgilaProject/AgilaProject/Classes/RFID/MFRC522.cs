...
using AgilaProject.RFID.MFRC522lib;
using static AgilaProject.RFID.MFRC522lib.MFRC522Registermap;

namespace AgilaProject.RFID
{
    class MFRC522
    {
        ...
        public async Task openMFRC522()
        {
            ...
            TransferToSpi(WRITE, (Byte)MFRC522Registermap.PCD_Register.DivIEnReg, 0x14); //IRQ is a st
            TransferToSpi(WRITE, (Byte)MFRC522Registermap.PCD_Register.DivIrqReg, 0x80); //IRQ is a st
            TransferToSpi(WRITE, (Byte)MFRC522Registermap.PCD_Register.ComIEnReg, 0xA0); //IRQ is a st            
            TransferToSpi(WRITE, (Byte)MFRC522Registermap.PCD_Register.ComIrqReg, 0x80); //IRQ is a st
      ...
      */
        ...
        public Byte MIFARE_Read(Byte blockAddr,         /* MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The first page to return data from. */
                                ...
                                ref Byte bufferSize)
        {   /* Buffer size, at least 18 bytes. Also number of bytes returned if STATUS_OK. */
        ...
        */
                                            ...
                                            byte bufferSize)
        {
            ...
            buffer[1] = (blockAddr);
            for (var idx = 0; idx < writeData.Length - 1; ++idx)
                buffer[idx + 2] = writeData[idx];

            result = PCD_MIFARE_Transceive(ref buffer, 6, false);
        ...
        private Byte PICC_REQA_or_WUPA(Byte command,    		    ///< The command to send - PICC_CMD_REQA or PICC_CMD_WUPA
									   ...
									   ref Byte bufferSize)
        {       ///< Buffer size, at least two bytes. Also number of bytes returned if STATUS_OK.
        ...
        private Byte PCD_TransceiveData(ref byte[] sendData,	        ///< Pointer to the data to transfer to the FIFO.
										...
										bool checkCRC = false)
        {        ///< In: True => The last two bytes of the response is assumed to be a CRC_A that must be validated.
            // RxIRq and IdleIRq
            Byte waitIRq = 0x30;
